...
/* ---------- CHIP AKURAT ---------- */
function fixChips(){if(!(state.session&&state.session.role==="wali"))return;var wrap=document.querySelector(".wb-chips");if(!wrap)return;var s=state.santri.find(function(x){return x.id===state.session.santriId;});if(!s)return;var tas=taListOf(s.id);var ta=state.waliTA||(tas.indexOf(currentTA())>=0?currentTA():latestTA(s.id));var L=0,S=0,B=0;var sp=getSpp(s.id,ta);if(sp)MONTHS.forEach(function(m){var c=sp.months[m];if(!c||!c.t)return;var st2=mStatus(c);if(st2==="lunas")L++;else if(st2==="seb")S++;else B++;});state.items.forEach(function(i){if(i.santriId===s.id&&i.ta===ta&&i.tarif>0){if(i.terbayar>=i.tarif)L++;else if(i.terbayar>0)S++;else B++;}});wrap.innerHTML='<span class="wb-chip">🟢 Lunas '+L+'</span><span class="wb-chip">🟠 Sebagian '+S+'</span><span class="wb-chip">⚪ Belum '+B+'</span>';}
/* ---------- TREN ---------- */
...
c.textAlign="center";c.fillStyle="#9BB5BA";c.font="400 12px 'Segoe UI',Arial";c.fillText("Kwitansi ini dibuat otomatis oleh Portal Administrasi DQH AL-HUDA — sah tanpa stempel & tanda tangan basah.",W/2,H-64);
showModal(cv,pay,s);};}
...
/* ---------- MESIN ---------- */
var lastLen=null;
function postRender(){injectControls();fixDemoBox();fixChips();injectTrend();injectAdminKw();injectRiwayatKw();maybeNotify();var n=state.pembayaran.length;if(lastLen===null){lastLen=n;}else if(n>lastLen&&state.session&&state.session.role==="admin"){var p=state.pembayaran[state.pembayaran.length-1];if(p)buatKwitansi(p);}lastLen=n;}
